...
async function exportData(api) {
  const [habits, entries, projects] = await Promise.all([api.listHabits(), api.listEntries(), api.listProjects()]);
  return { version: 1, exportedAt: new Date().toISOString(), habits, entries, projects };
}
...
async function importData(api, data) {
  ...
  for (const e of entries) await api.setEntry({ habitId: e.habitId, date: e.date, value: e.value ?? 0, note: e.note ?? "" });
...
export default function SettingsPage() {
  ...
  async function loadPack(pack) {
    ...
    for (const p of pack.projects) await api.upsertProject({ ...p, habitIds: habits.slice(0, 3).map((x) => x.id) });
  ...
  return (
    <div className="stack">
      <div className="card">
        <h2>Settings</h2>
        <p className="subtle">Load real-life examples, export your data, or import it on another device.</p>
      </div>
              ...
              <div className="row between" style={{ gap: 14 }}>
                <div className="stack" style={{ gap: 4, minWidth: 0 }}>
                  ...
                  <div className="subtle">
                    Habits: {pack.habits.length} · Projects: {pack.projects.length}
                  </div>
                </div>
                <button className="btn primary" type="button" onClick={() => loadPack(pack)}>
                  Load
                </button>
              </div>
            </div>
      ...
      <div className="card">
        ...
        <div className="row" style={{ flexWrap: "wrap" }}>
          ...
          <button
            ...
            type="button"
            onClick={async () => {
              const ok = window.confirm("This wipes all local data for this app in this browser. Continue?");
              if (!ok) return;
        ...
        <p className="subtle">Data is stored locally in your browser (IndexedDB). Export regularly if it matters.</p>
      </div>
    </div>
  );
}
