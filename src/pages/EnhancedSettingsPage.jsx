...
async function exportData(api, { supabaseConfigured, user }) {
	const [
		habits,
		entries,
		projects,
		...
		api.listHabitSessions?.() ?? [],
		api.listDailyReviews?.() ?? [],
	...
	return {
		...
		source: supabaseConfigured ? 'supabase' : 'local',
...
export default function EnhancedSettingsPage() {
	const { api, isReady, supabaseConfigured, user, session, authLoading } =
		useApp();
	...
	const handleThemeToggle = () => {
		...
		toast.push(`Switched to ${newDark ? 'dark' : 'light'} mode`);
	};

	const handleExportData = async () => {
		if (authLoading) {
			toast.push('Wait for account verification before exporting data.');
			return;
		}

		if (supabaseConfigured && (!session || !user)) {
			toast.push('Sign in before exporting synced account data.');
			return;
		}

		if (supabaseConfigured && api?.userId && api.userId !== user.id) {
			toast.push('Export blocked because the active account does not match.');
			return;
		}

		const confirmed = window.confirm(
			supabaseConfigured
				? `Export all habit data for ${user.email ?? user.id}? The JSON file may include notes, reviews, linked playlists, and settings.`
				: 'Export all local habit data from this browser? The JSON file may include notes, reviews, linked playlists, and settings.',
		);
		if (!confirmed) return;

		setExportLoading(true);
		try {
			const data = await exportData(api, { supabaseConfigured, user });
			...
			toast.push('Data exported successfully!');
		} catch (error) {
			toast.push('Failed to export data');
		} finally {
				...
				>
					<div className="space-y-4">
						<button
							...
							disabled={exportLoading || authLoading}
						...
						<p className="text-xs text-neutral-600 dark:text-neutral-400">
							Exports require confirmation. Synced exports are limited to the
							current signed-in account; local exports include only data stored in
							this browser.
						</p>
					</div>
				</SettingsSection>
