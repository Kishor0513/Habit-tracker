...
export class SupabaseHabitApi {
  ...
  async upsertHabit(habit) {
    ...
    const { data, error } = await this.supabase.from("habits").upsert(payload).select("*").single();
    must(!error, error);
  ...
  async upsertProject(project) {
    ...
    const { data, error } = await this.supabase.from("projects").upsert(payload).select("*").single();
    must(!error, error);
    return normalizeProject(data);
  }

  async deleteProject(projectId) {
    const { error } = await this.supabase.from("projects").delete().eq("id", projectId).eq("user_id", this.userId);
    must(!error, error);
  }
